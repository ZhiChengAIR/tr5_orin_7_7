...
void StateEst::init(std::shared_ptr<DataBus>& Data)
{
    offYaw = Data->rpy[2];
    ...
    KF_Q_wPCoM << 5e-2, 5e-2, 5e-2;
    // KF_Q_wVCoM << 1e-7,1e-7,1e-6;
    KF_Q_wVCoM << 1e-2, 1e-2, 1e-2;
    ...
    KF_R_wh = Eigen::Matrix<double, 2, 1>::Ones() * 1e-9;
    pos_old = Eigen::Vector3d::Zero();
    sim_time_old = 0.0;
    roll_last = 0.0;
    yaw_last = 0.0;
    pitch_last = 0.0;
...
void StateEst::set(std::shared_ptr<DataBus>& Data)
{
    if (flag_init)
        offYaw = Data->rpy[2];
...
void StateEst::update()
{
    ...
    diagR.segment<6>(6).setConstant(1.0);
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        double z_target = leg_contact[i] ? 0.07 : (base_pos(2) + pbW.col(i)(2));
        z_filtered[i] = 0.95 * z_filtered[i] + 0.05 * z_target;
        Y(12 + i) = z_filtered[i];
    }
    ...
    //std::cout << "base_vel " << base_vel(0) << " " << base_vel(1) << " " << base_vel(2) << std::endl;
...
void StateEst::get(double simtime, std::shared_ptr<DataBus>& Data)
{
    ...
    Data->dq.block<3, 1>(0, 0) = (Data->q.block<3, 1>(0, 0) - pos_old) / (simtime - sim_time_old);
    //double alpha = 0.2; // 0~1之间，越小越平滑，越大越快

    //Eigen::Vector3d vel_raw = (Data->q.block<3,1>(0,0) - pos_old) / (simtime - sim_time_old);

    //vel_filt = alpha * vel_raw + (1 - alpha) * vel_filt;

    //Data->dq.block<3, 1>(0, 0) = vel_filt;

    pos_old = Data->q.block<3, 1>(0, 0);
    sim_time_old = simtime;
...
void StateEst::setF(std::shared_ptr<DataBus>& Data)
{
...
void StateEst::updateF(std::array<std::array<double, 6>, 2> force_data_in)
{
...
//	std::cout << "state est FLest " << FLest[i] << std::endl;
//	std::cout << "state est FRest " << FRest[i] << std::endl;
    }
    //FLest = force_data_in[0];
    //FRest = force_data_in[1];
}

void StateEst::getF(std::shared_ptr<DataBus>& Data)
{
    Data->FL_est = FLest;
    Data->FR_est = FRest;
    //FLest.clear();
