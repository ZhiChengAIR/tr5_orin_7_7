...
public:
    ...
    bool get_init();
    void init(std::shared_ptr<DataBus>& Data); // set X0 and P0
    void set(std::shared_ptr<DataBus>& Data);
    void update();
    void get(double simtime, std::shared_ptr<DataBus>& Data);
    void setF(std::shared_ptr<DataBus>& Data);
    void updateF(std::array<std::array<double, 6>, 2> force_data_in);
    void getF(std::shared_ptr<DataBus>& Data);
//private:
    double dt;
    bool flag_init = true;
...
//    Eigen::Matrix<double, 3,1> pBR, pBL;
    ...
    std::array<double, 2> z_filtered = {0.07, 0.07};
