...
void WBC_priority::dataBusRead(const std::shared_ptr<DataBus>& robotState) {
...
void WBC_priority::dataBusWrite(std::shared_ptr<DataBus>& robotState) {
...
//    qpOASES::real_t xOpt_iniGuess[QP_nv];
    ...
    //std::cout << "eigen_qp_H " << eigen_qp_H << std::endl;
    //std::cout << "eigen_qp_A_final " << eigen_qp_A_final << std::endl;
    //std::cout << "eigen_qp_lbA " << eigen_qp_lbA << std::endl;
    //std::cout << "eigen_qp_ubA " << eigen_qp_ubA << std::endl;
...
//        printf("WBC-QP: init_failed\n");
    ...
    //std::cout << "Fr_ff " << Fr_ff << std::endl;
    //std::cout << "eigen_xOpt " << eigen_xOpt.block<12, 1>(6, 0) << std::endl;
...
void WBC_priority::computeDdq(const std::shared_ptr<Pin_KinDyn>& pinKinDynIn) {
    // task definition
    {
...
//            kin_tasks_walk.taskLib[id].dJ.block(0,22,3,3).setZero();
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(4, 4) * 30;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(4, 4) * 3;
	...
	//std::cout << "in wbcccc err x " << kin_tasks_walk.taskLib[id].errX(3) << std::endl;
        kin_tasks_walk.taskLib[id].derrX = Eigen::VectorXd::Zero(4);
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(2, 2) * 30;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(2, 2) * 3;
        ...
        kin_tasks_walk.taskLib[id].kp.block(3,3,3,3)=Eigen::MatrixXd::Identity(3, 3) * 30;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 1;
        kin_tasks_walk.taskLib[id].kd.block(3,3,3,3)=Eigen::MatrixXd::Identity(3, 3) * 3;
        ...
        kin_tasks_walk.taskLib[id].errX = Eigen::VectorXd::Zero(6);
        kin_tasks_walk.taskLib[id].errX.block<3, 1>(0, 0) = (swing_fe_pos_des_W - fe_pos_sw_W) / 2.0;
...
//        kin_tasks_walk.taskLib[id].derrX=-Jsw*dq;
        kin_tasks_walk.taskLib[id].ddxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_walk.taskLib[id].dxDes = Eigen::VectorXd::Zero(6);
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(6, 6) * 30;
        kin_tasks_walk.taskLib[id].kp.block<1, 1>(2, 2) = kin_tasks_walk.taskLib[id].kp.block<1, 1>(2, 2) * 0.1;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(6, 6) * 3;
        ...
        Eigen::Vector3d shoulder2hand_l_pos_L_des = { -0.0500,  0.25, -0.63};
        Eigen::Vector3d shoulder2hand_r_pos_L_des = { -0.0500, -0.25, -0.63};
        ...
        //kin_tasks_walk.taskLib[id].errX.block<3, 1>(0, 0) = Eigen::VectorXd::Zero(3);
	//kin_tasks_walk.taskLib[id].errX.block<3, 1>(3, 0) = diffRot(hd_l_rot_cur_W, hd_l_rot_W_des);
        ...
        //kin_tasks_walk.taskLib[id].errX.block<3, 1>(6, 0) = Eigen::VectorXd::Zero(3);
	//kin_tasks_walk.taskLib[id].errX.block<3, 1>(9, 0) = diffRot(hd_r_rot_cur_W, hd_r_rot_W_des);
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(12, 12) * 0;
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(12, 12) * 0;
        ...
        kin_tasks_walk.taskLib[id].kp = Eigen::MatrixXd::Identity(14, 14) * 0; //100
        kin_tasks_walk.taskLib[id].kd = Eigen::MatrixXd::Identity(14, 14) * 0;
	...
	kin_tasks_stand.taskLib[id].W.diagonal() = Eigen::VectorXd::Ones(model_nv);
        ...
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(3, 3) * 100;
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(3, 3) * 10;
        ...
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(1, 1) * 100; //100
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(1, 1) * 3;
        ...
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(2, 2) * 100;
	...
	//std::cout << "pCoMDes " << pCoMDes(0) << " " << pCoMDes(1) << " " << pCoMDes(2) << std::endl;
	//std::cout << "pCoMCur " << pCoMCur(0) << " " << pCoMCur(1) << " " << pCoMCur(2) << std::endl;
//            kin_tasks_stand.taskLib[id].errX[0]+=0.01;
	...
	//std::cout << "errX newww " << kin_tasks_stand.taskLib[id].errX << std::endl;
...
//            kin_tasks_stand.taskLib[id].derrX.block(2,0,3,1)=-taskMapRPY*J_hip_link*dq;
        ...
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(5, 5) * 50; //100
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(5, 5) * 5;
        kin_tasks_stand.taskLib[id].kp.block(2,2,3,3) = Eigen::MatrixXd::Identity(3, 3) * 50;
        kin_tasks_stand.taskLib[id].kd.block(2,2,3,3) = Eigen::MatrixXd::Identity(3, 3) * 5;
        ...
        Eigen::Vector3d base2shoulder_l_pos_L_des = {-0.0100,  0.11575,  0.36217};
        Eigen::Vector3d base2shoulder_r_pos_L_des = {-0.0100, -0.11575,  0.36217};
        Eigen::Vector3d shoulder2hand_l_pos_L_des = { 0.0100,  0.25, -0.66};
        Eigen::Vector3d shoulder2hand_r_pos_L_des = { 0.0100, -0.25, -0.66};
        ...
        kin_tasks_stand.taskLib[id].kp = Eigen::MatrixXd::Identity(14, 14) * 100; //100
        kin_tasks_stand.taskLib[id].kd = Eigen::MatrixXd::Identity(14, 14) * 10;
