...
void Pin_KinDyn::dataBusRead(const std::shared_ptr<DataBus>& robotState) {
...
void Pin_KinDyn::dataBusWrite(std::shared_ptr<DataBus> robotState) {
    robotState->J_l = J_l;
    robotState->J_r = J_r;
...
Pin_KinDyn::computeInK_Leg(const Eigen::Matrix3d &Rdes_L, const Eigen::Vector3d &Pdes_L, const Eigen::Matrix3d &Rdes_R,
    ...
    for (itr_count=0;; itr_count++)
    {
        ...
        {
            success = false;
            //std::cout << "unreached leg" << std::endl;
            break;
        }
...
Pin_KinDyn::computeInK_Hand(const Eigen::Matrix3d &Rdes_L, const Eigen::Vector3d &Pdes_L, const Eigen::Matrix3d &Rdes_R,
    ...
    for (itr_count=0;; itr_count++)
    {
        ...
        const pinocchio::SE3& oMi_L = data_biped_fixed.oMi[J_Idx_l];  // Left hand transformation matrix
        const pinocchio::SE3& oMi_R = data_biped_fixed.oMi[J_Idx_r];  // Right hand transformation matrix

        // Extract positions (translations) from the transformation matrices
        Eigen::Vector3d pos_L = oMi_L.translation();  // Position of left hand
        Eigen::Vector3d pos_R = oMi_R.translation();  // Position of right hand

        Eigen::Matrix3d rot_L = oMi_L.rotation();  // Rotation of left hand
        Eigen::Matrix3d rot_R = oMi_R.rotation();  // Rotation of right hand

        // Convert rotation matrices to Euler angles (roll, pitch, yaw)
        Eigen::Vector3d euler_L = rot_L.eulerAngles(0, 1, 2);  // (roll, pitch, yaw) for left hand
        Eigen::Vector3d euler_R = rot_R.eulerAngles(0, 1, 2);

        // Print the position coordinates (x, y, z)
        // if (itr_count < 100) {
        //     std::cout << "Left Hand Position: x = " << pos_L.x() << ", y = " << pos_L.y() << ", z = " << pos_L.z() << std::endl;
        //     std::cout << "Left Hand Rotation (Euler angles): roll = " << euler_L.x() << ", pitch = " << euler_L.y() << ", yaw = " << euler_L.z() << std::endl;
        //     std::cout << "Right Hand Position: x = " << pos_R.x() << ", y = " << pos_R.y() << ", z = " << pos_R.z() << std::endl;    
        //     std::cout << "Right Hand Rotation (Euler angles): roll = " << euler_R.x() << ", pitch = " << euler_R.y() << ", yaw = " << euler_R.z() << std::endl;
        // }
        const pinocchio::SE3 iMdL = data_biped_fixed.oMi[J_Idx_l].actInv(oMdesL);
        const pinocchio::SE3 iMdR = data_biped_fixed.oMi[J_Idx_r].actInv(oMdesR);
        errL = pinocchio::log6(iMdL).toVector();  // in joint frame
        ...
        {
            success = false;
            //std::cout << "unreached hand" << std::endl;
            break;
        }
