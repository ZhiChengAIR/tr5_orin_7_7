...
mjModel* mj_model = mj_loadXML("../models/scene_board.xml", 0, error, 1000);    // This part loads the MuJoCo model from an XML file (scene_board.xml) and creates simulation data (mj_data).
...
int main(int argc, const char** argv)
{
    ...
    double foot_height = 0.07; // distance between the foot ankel joint and the foot bottom
    double  xv_des = 1.2;  // desired velocity in x direction，原来是1.2

    RobotState.width_hips = 0.24;           // 髋关节：原来0.229
    footPlacement.kp_vx = 0.03;             //原来是0.03
    footPlacement.kp_vy = 0.035;
    footPlacement.kp_wz = 0.03;
    footPlacement.stepHeight = 0.3;            // 抬脚高度：原来0.25
    ...
    // Eigen::Vector3d hd_l_eul_L_des={0.5, 0.3, 0.38};
    // Eigen::Vector3d hd_r_eul_L_des={0.5, -0.3, 0.38};
    ...
    double startSteppingTime=0.8;
    double startWalkingTime=1.2;
    ...
    while( !glfwWindowShouldClose(uiController.window))
    {
        ...
        while( mj_data->time - simstart < 1.0/60.0 && uiController.runSim) // press "1" to pause and resume, "2" to step the simulation
        {
            ...
            if (simTime<=startSteppingTime){
                ...
                // for (i=0, i < RobotState.motors_pos_des.size(),i++)
                // std::cout << "RobotState.motors_pos_des: " << RobotState.motors_pos_des[i] << std::endl;
