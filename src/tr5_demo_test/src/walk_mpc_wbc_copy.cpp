...
class RobotControlNode : public rclcpp::Node {
    public:
	    ...
	    feet_forces_old[0].fill(0.0);
            ...
            std::cout << "test 11112222" << std::endl;
            ...
            fe_l_pos_L_des={-0.05, 0.13, -stand_legLength};
            fe_r_pos_L_des={-0.05, -0.13, -stand_legLength};
	    ...
	    test_motor = 0;
            ...
            simTime = 0;
            startSteppingTime=50;
            startWalkingTime=50.5;
            simEndTime=60000;
	    ...
	    start_time_2 = this->get_clock()->now();
	...
	//	std::cout << "aaaaaaaaaaaaa" << std::endl;
	    ...
	    for (int i=0;i<model_nv-6;i++)
            {
		if (i <= 13) {
		    robotState_->q(i+7) = 0.0;
		} else {
	 	    robotState_->q(i+7)=robotState_->motors_pos_cur[i];
		}
               // robotState_->q(i+7)=robotState_->motors_pos_cur[i];
	//	std::cout << "robotState_->q(i+7) " << robotState_->q(i+7) << std::endl;
            }
            for (int i=0;i<model_nv-6;i++)
            {
		if (i <= 13) {
                    robotState_->dq(i+6) = 0.0;
                } else {
                    robotState_->dq(i+6)=robotState_->motors_vel_cur[i];
                }
		   ...
		   // for (int i = 0; i < 30; i++) {
		    ...
		    //std::cout << "##########################" << std::endl;
			    ...
			    //if ((current_time - last_print_time).seconds() >= 1.0) {
                		//RCLCPP_INFO(this->get_logger(), "Elapsed time: %.2f seconds", elapsed_time.seconds());
                		//last_print_time = current_time;
		           // std::cout << "robotState_->motors_pos_des " << robotState_->motors_pos_des[i] << std::endl;
			   // std::cout << "robotState_->q " << robotState_->q[i+7] << std::endl;
			    //}
...
//            printf("basePos=[%.5f, %.5f, %.5f]\n", RobotState.basePos[0], RobotState.basePos[1], RobotState.basePos[2]);
		...
		logger_->recItermData("com_cur",robotState_->pCoM_W);	
		logger_->finishLine();
