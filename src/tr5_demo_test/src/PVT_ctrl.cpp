...
PVT_Ctr::PVT_Ctr(double timeStepIn, const char *jsonPath) {
    ...
    maxTor.assign(jointNum,720);
    maxVel.assign(jointNum,63);
...
void PVT_Ctr::dataBusRead(const std::shared_ptr<DataBus>& busIn) {
    for (int i = 0; i < jointNum; i++) {
...
void PVT_Ctr::dataBusWrite(std::shared_ptr<DataBus>& busIn) {
...
void PVT_Ctr::calMotorsPVT() {
    for (int i=0;i<jointNum;i++)
    {
        ...
        //tauDes=tau_out_lpf[i].ftOut(tauDes);
...
const double vel_tol = 0.01;

double pos_err = pDes - motor_pos_cur[i];
double vel_err = motor_vel_des[i] - motor_vel[i];

if (std::abs(pos_err) < pos_tol) {
    tauDes = 0.0;
    //std::cout << "in tol" << std::endl;
} else {
    //std::cout << "out tol" << std::endl;
    double adj_pos_err = pos_err;
    if (pos_err > 0)
        adj_pos_err -= pos_tol;
    else if (pos_err < 0)
        adj_pos_err += pos_tol;
    ...
    //double adj_vel_err = vel_err;
    //if (vel_err > 0)
      //  adj_vel_err -= vel_tol;
    //else if (vel_err < 0)
      //  adj_vel_err += vel_tol;

    //tauDes = PV_enable[i] * pvt_Kp[i] * adj_pos_err +
      //       PV_enable[i] * pvt_Kd[i] * vel_err;
}	
        tauDes=PV_enable[i]*pvt_Kp[i]*(pDes-motor_pos_cur[i])+PV_enable[i]*pvt_Kd[i]*(motor_vel_des[i]-motor_vel[i]);
	
       //std::cout << "real kp kd " << pvt_Kp[i] << " " << pvt_Kd[i] << std::endl;
	//std::cout << "real pos " << motor_pos_cur[i] << std::endl;
	//std::cout << "delta pos " << motor_pos_des[i] - motor_pos_cur[i] << std::endl;
	//std::cout << "tauDes " << tauDes << std::endl;
	//std::cout << "delta vel " << motor_vel_des[i]-motor_vel[i] << std::endl;
	//std::cout << "motor_tor_des " << +motor_tor_des[i] << std::endl;
	tauDes=tau_out_lpf[i].ftOut(tauDes)+motor_tor_des[i];
	//tauDes=tau_out_lpf[i].ftOut(tauDes);
        if (fabs(tauDes)>=fabs(maxTor[i]))
            tauDes= sign(tauDes)*maxTor[i];
