...
// func eul2Rot(r,p,y): converts Euler angles (roll, pitch, yaw) to a rotation matrix.
...
Eigen::Matrix<double, 3, 1> diffRot(const Eigen::Matrix3d &Rcur, Eigen::Matrix3d &Rdes) {
    Eigen::Matrix3d R = Rcur.transpose() * Rdes;    // 作用：计算当前旋转矩阵与目标旋转矩阵的相对旋转 R。Rcur.transpose() 是当前旋转矩阵的转置，将其与目标旋转矩阵相乘得到相对旋转矩阵。这个矩阵 R 表示当前姿态与目标姿态之间的旋转差异。
    Eigen::Vector3d w;
