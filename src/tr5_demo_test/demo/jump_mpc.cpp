...
int main(int argc, char **argv) {
    ...
    // variables ini
    double stand_legLength = 1.08;  // desired baselink height
    ...
    Eigen::Vector3d fe_l_pos_L_des = {-0.017, 0.13, -stand_legLength};
    Eigen::Vector3d fe_r_pos_L_des = {-0.017, -0.13, -stand_legLength};
    ...
    double prepareTime = 0.3;
    double startJumpingTime = 3; // start jumping time
    uint16_t jump_state = 0;
    double stand_z = -0.86;     // 下蹲的高度（相对base_link），og: stand_z = -0.8;
    double jump_z = 0.14;       // 起跳高度
    double count = 0.0;
    ...
    while (!glfwWindowShouldClose(uiController.window)) {
        ...
        while (mj_data->time - simstart < 1.0 / 60.0 && uiController.runSim) {
            ...
            std::cout << " ---------------- Desire motors_vel_des -------------------- " << RobotState.motors_vel_des.size() <<std::endl;
            printf("Waist_motors_vel_des (roll、yaw)= [%.5f, %.5f]\n", RobotState.motors_vel_des[16], RobotState.motors_vel_des[17]);
            printf("hip_l_motors_vel_des (roll、yaw、pitch) = [%.5f, %.5f, %.5f]\n", RobotState.motors_vel_des[18], RobotState.motors_vel_des[19], RobotState.motors_vel_des[20]);
            printf("hip_r_motors_vel_des (roll、yaw、pitch) = [%.5f, %.5f, %.5f]\n", RobotState.motors_vel_des[24], RobotState.motors_vel_des[25], RobotState.motors_vel_des[26]);
            printf("knee_l, knee_r_motors_vel_des = [%.5f, %.5f]\n", RobotState.motors_vel_des[21], RobotState.motors_vel_des[27]);
            printf("ankle_l_motors_vel_des (pitch、 roll) =[%.5f, %.5f]\n", RobotState.motors_vel_des[22], RobotState.motors_vel_des[23]);
            printf("ankle_r_motors_vel_des (pitch、 roll) =[%.5f, %.5f]\n", RobotState.motors_vel_des[28], RobotState.motors_vel_des[29]);
            ...
            printf("hip_l_motors_vel_cur (roll、yaw、pitch) = [%.5f, %.5f, %.5f]\n", RobotState.motors_vel_cur[18], RobotState.motors_vel_cur[19], RobotState.motors_vel_cur[20]);
            printf("hip_r_motors_vel_cur (roll、yaw、pitch) = [%.5f, %.5f, %.5f]\n", RobotState.motors_vel_cur[24], RobotState.motors_vel_cur[25], RobotState.motors_vel_cur[26]);
            ...
            printf("hip_l_motors_pos_cur (roll、yaw、pitch) = [%.5f, %.5f, %.5f]\n", RobotState.motors_pos_cur[18], RobotState.motors_pos_cur[19], RobotState.motors_pos_cur[20]);
            printf("hip_r_motors_pos_cur (roll、yaw、pitch) = [%.5f, %.5f, %.5f]\n", RobotState.motors_pos_cur[24], RobotState.motors_pos_cur[25], RobotState.motors_pos_cur[26]);
