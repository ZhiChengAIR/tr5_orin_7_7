...
int main(int argc, const char** argv)
{
    ...
    Pin_KinDyn kinDynSolver("../models/TR5.urdf"); // kinematics and dynamics solver 动力学求解器
    DataBus RobotState(kinDynSolver.model_nv); // data bus
    WBC_priority WBC_solv(kinDynSolver.model_nv, 18, 22, 0.7, mj_model->opt.timestep); // WBC solver
    GaitScheduler gaitScheduler(0.35, mj_model->opt.timestep); // gait scheduler 步态调度器
    ...
    while( !glfwWindowShouldClose(uiController.window))
    {
        ...
        while( mj_data->time - simstart < 1.0/60.0 && uiController.runSim) // press "1" to pause and resume, "2" to step the simulation
        {
            ...
            RobotState.base_pos_des(2) = stand_legLength + foot_height + (RobotState.basePos[0] - 0.025)*0.04;
