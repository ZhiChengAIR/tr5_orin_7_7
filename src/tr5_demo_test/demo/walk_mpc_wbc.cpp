...
int main(int argc, char **argv) {
    ...
    DataBus RobotState(model_nv); // data bus
    Mock_Interface mj_interface(model_nv - 6);
    WBC_priority WBC_solv(model_nv, 18, 22, 0.7, dt);
    MPC MPC_solv(dt_200Hz);  // mpc controller
    GaitScheduler gaitScheduler(0.35, dt);
    PVT_Ctr pvtCtr(dt,"../common/joint_ctrl_config.json");
    FootPlacement footPlacement; // foot-placement planner
    JoyStickInterpreter jsInterp(dt);
    DataLogger logger("../record/datalog_local.log"); // data logger
    CSVInputLoader loader("../build/mujoco_log.csv", 30);
    ...
    Eigen::VectorXd qIniDes = Eigen::VectorXd::Zero(model_nv + 1);
    ...
    double simEndTime=2;
        ...
        while (simTime < simEndTime) { // press "1" to pause and resume, "2" to step the simulation
            printf("-------------%.3f s------------\n",simTime);
            // Read the sensors:
            if (!loader.nextFrame(frame)) {
            ...
            mj_interface.loadFromFrame(frame);
            ...
            mj_interface.logToCSV(simTime, RobotState.motors_tor_out, eigen2std(RobotState.wbc_tauJointRes), RobotState.motors_tor_des);
...
//            printf("basePos=[%.5f, %.5f, %.5f]\n", RobotState.basePos[0], RobotState.basePos[1], RobotState.basePos[2]);
            ...
            logger.recItermData("motor_tor_des", RobotState.motors_tor_out);
            logger.recItermData("motor_tor_out", RobotState.wbc_tauJointRes);
...
/*             printf("rpyVal=[%.5f, %.5f, %.5f]\n", RobotState.rpy[0], RobotState.rpy[1], RobotState.rpy[2]);
            ...
            if (fabs(simTime - 0.160) < 1e-5) {
                ...
                std::cout << "[DEBUG] base_rpy = " << RobotState.base_rpy.transpose() << std::endl;
                std::cout << "[DEBUG] base_rot:\n" << RobotState.base_rot << std::endl;
                std::cout << "[DEBUG] norm(R - I) = " 
                          << (RobotState.base_rot - Eigen::Matrix3d::Identity()).norm() << std::endl;
                isRotationIdentity(RobotState.base_rot);
            }
